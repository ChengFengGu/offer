...
# S(i-1) = 1 输入:S(i) = 0 的时候, f不满足条件 g(i) = min(f(i-1),g(i-1)) + 1
# S(i-1) = 1 输入:S(i) = 1 的时候, f不满足条件 g(i) = min(f(i-1),g(i-1))

# 以00110 为例:

# i = 0 时 输入为0: f(0) = 0 g(0) = 1
# i = 1 时 输入为0 S(i-1) 为 0 : f(1) = f(0) = 0 , g(1) = min(0,1) + 1 = 1
# i = 2 时 输入为1 S(i-1) 为 0 : f(2) = f(1) + 1 = 1 , g(1) = min(0,1) = 0
# i = 3 时 输入为1 S(i-1) 为 1 : 
